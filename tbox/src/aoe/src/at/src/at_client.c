...
int at_obj_exec_cmd(at_client_t client, at_response_t resp, const char *cmd_expr, ...)
{
    ...
    va_end(args);
	//LOG_D("at_obj_exec_cmd at_vprintfln finish");
    if (resp != RT_NULL)
    {
    	//LOG_D("at_obj_exec_cmd wait resp");
        resp->line_counts = 0;
		...
		//LOG_D("at_obj_exec_cmd  resp:%d", client->resp_status);
...
static rt_err_t at_client_getchar(at_client_t client, char *ch, rt_int32_t timeout)
{
    ...
    while (rt_device_read(client->device, 0, ch, 1) == 0)
    {
		...
		//LOG_D("at_client_getchar SLEEP0"); //debug
		tb_msleep(UART_READ_POLL_TIME_MS);

		#if 0
		left_time -= UART_READ_POLL_TIME_MS;
		if(left_time <= 0)
		{
			//LOG_D("at_client_getchar timeout");
			return RT_ETIMEOUT;
		}
		#endif

		//LOG_D("at_client_getchar SLEEP1"); //debug
		#else
...
static int at_recv_readline(at_client_t client)
{
    ...
    while (1)
    {
		...
		//LOG_D("at_client_getchar ch:%c", ch); //debug
		if(client->thread_exit_flag)
		    break;
        ...
        if ((ch == '\n' && last_ch == '\r') || (client->end_sign != 0 && ch == client->end_sign)
                || get_urc_obj(client))
        {
        	//LOG_D("at_client_getchar hit n/r"); //debug
            if (is_full)
            {
...
static void client_parser(at_client_t client)
{
	...
	//LOG_D("start client parser");
    while(1)
    {
    	if(client->thread_exit_flag)
			break;
    	//LOG_D("client parser1");
        if (at_recv_readline(client) > 0)
        {
            ...
            else if (client->resp != RT_NULL)
            {
                ...
                if (resp_buf_len + client->cur_recv_len < client->resp->buf_size)
                {
                    ...
                    line_counts++;
					//LOG_D("have read a line:%d", line_counts);
                }
                else
                {
                ...
                {
                	//LOG_D("have read a line hit error");
                ...
                client->resp = RT_NULL;
				//LOG_D("have read a line rt_sem_release resp_notice");
...
#define AT_CLIENT_THREAD_NAME          "at_clnt"
    ...
    rt_snprintf(name, RT_NAME_MAX_DUP, "%s%d", AT_CLIENT_LOCK_NAME, at_client_num);
    ...
    rt_snprintf(name, RT_NAME_MAX_DUP, "%s%d", AT_CLIENT_SEM_NAME, at_client_num);
    ...
    rt_snprintf(name, RT_NAME_MAX_DUP, "%s%d", AT_CLIENT_RESP_NAME, at_client_num);
    ...
    rt_snprintf(name, RT_NAME_MAX_DUP, "%s%d", AT_CLIENT_THREAD_NAME, at_client_num);
	#ifdef AOE_REFACTORING
	client->parser = rt_thread_create(name, client_parser, client, 0, 0, 0);
	#else
...
int at_client_init(const char *dev_name,  rt_size_t recv_bufsz)
{
	...
	#ifdef AOE_REFACTORING
	client->device = rt_find_device(dev_name);
	if (client->device == RT_NULL)
	{
...
int at_client_release(at_client_t client)
{
	if(client == NULL)
		return -1;

    {
        client->thread_exit_flag = 1; 
		tb_msleep(UART_READ_POLL_TIME_MS * 2);
    }
	if (client->lock)
    {
        rt_mutex_delete(client->lock);
