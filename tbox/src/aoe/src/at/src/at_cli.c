...
#ifdef AOE_REFACTORING
	console_rx_notice = rt_sem_create("console_rx_notice", 0, 0);
#else
...
#ifdef AOE_REFACTORING
	exit_at_debug_mode();
	rt_sem_delete(console_rx_notice);
#else
    int_lvl = rt_hw_interrupt_disable();
    console = rt_console_get_device();
...
static char client_getchar(void)
{
    char ch;
#ifdef AOE_REFACTORING
	at_client_t clent = at_client_get_first();

	while(clent->device->state != UART_AT_DEBUG || rt_device_read(clent->device, 0, &ch, 1) == 0)
	{
		tb_msleep(10);
	}
#else
...
#else
		rt_sem_create("client_rx_notice", 0, 0);
...
#else
			rt_sem_delete(client_rx_notice);
#endif
