...
FINSH_FUNCTION_EXPORT(aoe_exit, aoe exit);
MSH_CMD_EXPORT(aoe_exit, aoe exit);


#define PPP_RUNNING_THREAD_NAME "pppthread"
extern int lwipLoop(void *param);

long lwip_ppp(void)
{
	//check port
	
	if(rt_find_device((aoe_get_uart_opt())->port_name) == NULL)
	{
		rt_kprintf("Error: port not open\n");
		return -1;
	}

	//create thread
	rt_thread_create(PPP_RUNNING_THREAD_NAME, lwipLoop, NULL, 0, 0, 0);

    return 0;
}
FINSH_FUNCTION_EXPORT(lwip_ppp, lwip ppp test);
MSH_CMD_EXPORT(lwip_ppp, lwip ppp test);


#define TCP_RUNNING_THREAD_NAME "tcpthread"
extern int lwipTcpClient(void *param);
...
*/
	//create thread
	rt_thread_create(TCP_RUNNING_THREAD_NAME, lwipTcpClient, NULL, 0, 0, 0);
